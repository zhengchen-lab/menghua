...
esp_err_t
esp_lcd_new_panel_st7789_fix(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config,
...
typedef struct init_code_t {
    uint8_t command;
    uint8_t data[14];
    uint8_t length;
} init_code_t;

static const init_code_t converted_code[] = {
    // Exit Sleep
    {0x11, {}, 0},
    
    // Porch settings
    {0xB2, {0x01, 0x01, 0x00, 0x33, 0x33}, 5},
    ...
    // Positive gamma
    {0xE0, {0xF0, 0x03, 0x09, 0x08, 0x08, 0x04, 0x33, 0x54, 0x49, 0x39, 0x14, 0x14, 0x2E, 0x33}, 14},
    
    // Negative gamma
    {0xE1, {0xF0, 0x09, 0x0D, 0x0A, 0x09, 0x15, 0x32, 0x44, 0x49, 0x36, 0x12, 0x12, 0x2C, 0x34}, 14},
...
static esp_err_t panel_st7789_init(esp_lcd_panel_t *panel)
{
    ...
    for (int i = 0; i < sizeof(converted_code) / sizeof(converted_code[0]);
         i++) {
      if (converted_code[i].length == 0) {
        ESP_RETURN_ON_ERROR(
            esp_lcd_panel_io_tx_param(io, converted_code[i].command, NULL, 0),
            TAG, "io tx param failed");
      } else {
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io,
                                                      converted_code[i].command,
                                                      converted_code[i].data,
                                                      converted_code[i].length),
                            TAG, "io tx param failed");
      }
      // Some commands need a delay to process
      if (converted_code[i].command == 0x11) {
        vTaskDelay(pdMS_TO_TICKS(120));
      } else if (converted_code[i].command == 0x29) {
