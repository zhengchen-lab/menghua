...
#define TAG "BlufiHTTP"

HttpClient::HttpClient() : client_(nullptr), status_code_(0) {}
...
bool HttpClient::Open(const std::string& method, const std::string& url, const std::string& content) {
  ...
  if (status_code_ == 0) {
    ESP_LOGE(TAG, "Failed to get status code");
    Close();
    return false;
  }

  ESP_LOGI(TAG, "request successful, status_code: %d content_length:%lld", status_code_, content_length_);
  return true;
}
