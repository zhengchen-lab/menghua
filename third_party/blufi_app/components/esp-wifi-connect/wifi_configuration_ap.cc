    ...
    httpd_uri_t form_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            if (!this_->ConnectToWifi(ssid_str, password_str)) {
                cJSON_Delete(json);
                std::string errMsg = "{\"success\":false,\"error\":\"连接 wifi:" + ssid_str + "失败，请检查密码是否正确\"}";
                httpd_resp_send(req, errMsg.c_str(), HTTPD_RESP_USE_STRLEN);
                return ESP_OK;
            }
    ...
    httpd_uri_t reboot = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            xTaskCreate([](void *ctx) {
                ...
                // 执行重启
                // esp_restart();
                if (self->on_ap_done_cb_) self->on_ap_done_cb_();
...
void WifiConfigurationAp::WifiEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    ...
    if (event_id == WIFI_EVENT_AP_STACONNECTED) {
        ...
        if (self && self->on_ap_join_cb_) self->on_ap_join_cb_();
