...
public:
    ...
    void SetDoneCallBack(std::function<void()> cb) {on_ap_done_cb_ = cb;}
    void SetJoinCallBack(std::function<void()> cb) {on_ap_join_cb_ = cb;}
    void Start();
    void Stop();
    ...
    bool IsStarted() const { return start_.load(); }
private:
    // Private constructor
    WifiConfigurationAp();
    ~WifiConfigurationAp();
    ...
    std::function<void()> on_ap_done_cb_ = nullptr;
    std::function<void()> on_ap_join_cb_ = nullptr;
